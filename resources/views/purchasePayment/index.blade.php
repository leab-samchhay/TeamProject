        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header d-flex align-items-center justify-content-between border-bottom border-light">
                    ...
                    <div>
                        <a href="{{ route('purchasePayment.create') }}" class="btn btn-success bg-gradient"><i
                            ...
                            @if ($rows)
                                @foreach ($rows as $row)
                                    <tr>
                                        ...
                                        <td>Purchase #{{ $row->PurchaseID }}</td>
                                        <td>${{ number_format($row->TotalPayment, 2) }}</td>
                                        <td>{{ $row->PurchaseDate->format('Y-m-d') }}</td>
                                        <td class="pe-3">
                                            <div class="hstack gap-1 justify-content-end">
                                                <a href="{{ route('purchasePayment.edit', $row->id) }}"
                                                    class="btn btn-soft-success btn-icon btn-sm rounded-circle"> <i
                                                        class="ti ti-edit fs-16"></i></a>

                                                <form action="{{ route('purchasePayment.remove', $row->id) }}" method="POST"
